...
public class KcbCatalogFileExportServlet extends HttpServlet {
	
	/**
	 * 
	 */
	private static final long serialVersionUID = 6831810565504619433L;
	private final static Logger logger = Logger.getLogger(KcbCatalogFileExportServlet.class.getName());
	private final static String FILE_NAME = "SDP_KCB_CATALOG_EXPORT.csv";
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if (request.isUserInRole(UserManager.MONITOR) || request.isUserInRole(UserManager.OPSUISER) || request.isUserInRole(UserManager.OPSADMIN)) {
			try {
				// Retrieve data.
				KcbProductBean[] beans = KcbCatalogDBWrapper.getAllProducts();
				byte[] bytes = CsvExportUtil.getKcbCatalogExportCsvString(beans).getBytes();
