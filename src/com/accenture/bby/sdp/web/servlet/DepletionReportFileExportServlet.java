...
public class DepletionReportFileExportServlet extends HttpServlet {
	
	private final static Logger logger = Logger.getLogger(DepletionReportFileExportServlet.class.getName());
	private final static String FILE_NAME = "SDP_DEPLETION_REPORT_EXPORT.csv";
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if (request.isUserInRole(UserManager.MONITOR) || request.isUserInRole(UserManager.OPSUISER) || request.isUserInRole(UserManager.OPSADMIN)) {
			try {
				...
				byte[] bytes = CsvExportUtil.getDepletionReportExportCsvString(Arrays.asList(list), days).getBytes();
